...
// @author       airbash
// @description  取消点击链接后跳转到安全页面,减少操作时间;支持`CSDN`+`掘金`+`简书`+`知乎`+`知乎专栏`+`百度贴吧`+`开源中国`+`gitee`+`51CTO`+`360搜索`+`必应搜索`
// @match        *://link.csdn.net/*
					...
					if (!website.inter) {
						...
						for (item of items) {
							if (item.getAttribute("href").includes(handler.start)) {
								...
								} else if (handler.type == "item") {
									//从其他属性中获取链接
									item.setAttribute("href", item.getAttribute(handler.item));
								} else {
									//从文本中获取地址
