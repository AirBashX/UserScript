...
1. 绕过搜索引擎(百度、搜狗、360、必应)搜索结果中的重定向链接,直链访问原始网站(受搜索引擎的限制,现在仅支持部分结果);
2. 删除网站重定向到安全页面,减少操作步骤和响应时间;
3. 自动跳转中文文档:默认关闭,需要在`Tampermonkey`+`Violentmonkey`+`ScriptCat`+`Via`等油猴插件菜单中开启;
